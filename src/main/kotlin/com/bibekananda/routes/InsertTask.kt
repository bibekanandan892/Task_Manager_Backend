...
fun Route.insertTask(taskRepository: TaskRepository, app: Application) {
    authenticate("jwt-auth") {
        post(Endpoint.InsertTask.path) {
            val principal = call.authentication.principal<JWTPrincipal>()
            try {
                val subId = principal?.payload?.getClaim(Constants.SUB_ID)?.asString()
                val taskRequest = call.receive<Task>()
                if (subId != null) {
                    val tasks = taskRepository.insertTask(subId = subId,task= taskRequest)
