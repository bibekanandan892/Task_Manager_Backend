...
import com.bibekananda.data.model.task.DeleteRequest
...
fun Route.deleteTask(taskRepository: TaskRepository, app: Application) {
    authenticate("jwt-auth") {
        delete(Endpoint.DeleteTask.path) {
            val principal = call.authentication.principal<JWTPrincipal>()
            try {
                val subId = principal?.payload?.getClaim(Constants.SUB_ID)?.asString()
                val taskRequest = call.receive<DeleteRequest>()
                if (taskRequest.taskId != null && subId != null) {
                    val tasks = taskRepository.deleteTask(subId = subId, taskId = taskRequest.taskId)
